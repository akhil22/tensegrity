...
#        hop_cost = 400*(height-foot)
       ...
       # hip_height_cost3 = self.sim.data.get_body_xpos("torso1")[0]
        ...
        hip_height_cost = self.sim.data.get_body_xpos("torso1")[0]
...
#         hip_height_cost = 0
       ...
       # reward = 0 * lin_vel_cost - quad_ctrl_cost - quad_impact_cost - 10 * hip_height_cost + jcost*jcost*0
        reward = lin_vel_cost*50 + hip_height_cost - (sensor_data[2] > 0)*50 - (sensor_data[4] > 0)*50 + sensor_data[0]*0 - quad_impact_cost - (sensor_data[8] > 0)*50 - (sensor_data[10] > 0)*50 + sensor_data[6]*0 + 0.5*(jcost1*jcost2 < 0)
     #   reward = 5 * lin_vel_cost - quad_ctrl_cost - quad_impact_cost
        #reward = lin_vel_cost*100
        qpos = self.sim.data.qpos
        ob = self._get_obs()
       # done = sensor_data[2] > 0
        done = False
        DOF = self.model.body_dofnum
#         done = bool((qpos[2] < 1.0) or (qpos[2] > 2.0))
        return ob, reward, done, dict(reward_linvel=vel*100, height_rew=hip_height_cost, 
                                      reward_impact=20*(jcost1*jcost2 < 0), joint_cost = 20*jcost1*jcost2, DOF = DOF, sd = sensor_data)
